...
li_menu = soup.find("div", {"class": "header-menu"}).find("ul", {"id": "menu-menu"}).find_all("li", recursive=False)
for linIndex in li_menu:
    ...
    for k in otherSubNav:
        subMenu = k.find_all("li", recursive=False)
        for subcat_link in subMenu:
            a_link = subcat_link.find("a")
            a_text = (a_link.get("href"))
            if re.search(r'(?i)https:|/wheeled-excavators|#|/testimonials|/leaders', a_text):
                pass
            else:
                subcat = URL + a_text
                catlinks_lis.append(subcat)

    for sub in catlinks_lis:
